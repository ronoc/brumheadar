...
Elasticsearch.prototype.bulkCreate = function(payload, done){
  ...
  payload.each((entry) => {
    ...
    bulkInput.body.push(Object.reject(entry, ["_id", "_type", "_index"]));
  });
...
Elasticsearch.prototype.bulkMulti = function(payload, done){
  ...
  //console.log("bulkMulti size " + JSON.stringify(payload)); 
  ...
  self.elasticClient.bulk(bulkInput, (err, resp) => {
    if(err) self.emit("error", err, "bulkMulti");
    self.emit("finished","bulkMulti", timerId, bulkInput.body.length/2);
    done(err);
  });
};
